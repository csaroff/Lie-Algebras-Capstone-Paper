...
Abelian Lie Algebra's are not two step nilpotent.
...
by itself, it looks like a line, while $\fv$ by itself looks like a plane.
...
\end{pmatrix}
$$
So $L_{ij}$ is the output of the Lie Bracket on the ith basis vector with the
jth basis vector.
...
\\    Let
    ...
    then the insides must be equal.  i.e. $EJ_{z_k} = (L^k)^T$.
